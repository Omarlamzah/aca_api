...
class QuizControllerValidation extends Controller
{
    private function getNextQuiz()
    {
       // $userQuizToken = UserQuizToken::where("user_id", Auth::id())->with("quizpart")->latest()->first();

        //test
        $userQuizToken = UserQuizToken::where("user_id", Auth::id())
            ->with("quizpart")
            ->whereHas('quizpart', function ($query) {
                $query->where('type', 'validation');
            })
            ->latest()
            ->first();
        ...
        if ($userQuizToken) {
            ...
            return QuizPart::where("type", "validation")
                ->with('questions',"essai",)
                ->orderBy('PartID')
                ->first();
        }
    ...
    public function index($identify): JsonResponse
    {
        ...
        if ($nextQuizPart) {
            ...
            return response()->json([
                'gamestatus'=>"finishgame2validation",
                'error' => 'You are finished all quizzes by ' . ($totalScore ? $totalScore->score : '0') . ' points',
    ...
    // not used     public function startQuiz(): JsonResponse
    //    {
    //        $totalScore = Userscore::where("user_id", Auth::id())->first();
    //
    //        if ($totalScore->score < 50) {
    //            return response()->json([
    //                'error' => 'Your score is not valid to pass quiz validation',
    ...
    //
    //            return response()->json([
    //                'error' => 'You have finished all quizzes by ' . ($totalScore ? $totalScore->score : 0) . ' points',
    //                'totalScore' => $totalScore ? $totalScore->score : 0,
    //            ]);
    //        }
    //    }
